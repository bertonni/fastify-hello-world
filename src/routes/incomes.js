...
export async function incomesRoute(app) {
  app.post("/incomes", { preHandler: verifyJWT }, async (req, reply) => {
    try {
      ...
      return reply.status(200).send({ message: 'Receita incluída com sucesso' });
    } catch (error) {
      return reply.status(400).send({ message: 'Nâo foi possível incluir a receita.' })
    }
  });
  ...
  app.get("/incomes/:userId", { preHandler: verifyJWT }, async (req, reply) => {
    ...
    try {
      ...
      return reply.status(200).send({ message: 'Não foi possível buscar as receitas' });
    }
  })
}
